...
* C++ source files:
  * `include/mcdft-structure/mcdft-structure.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/mcdft-structure.cpp` is the main file that implements this library.
  * `app/mcdft-structure_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/mcdft-structure_t.cpp` contains the unit tests for the library.
  ...
  * The `python/mcdftstructure` directory contains a Python
    package for the project. It contains a compiled Python module `_mcdftstructure`
    that CMake generates from the Pybind11 source file `python/mcdftstructure/_mcdft-structure.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
